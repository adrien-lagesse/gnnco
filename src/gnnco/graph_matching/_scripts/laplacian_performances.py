...
import mlflow
import torch
from gnnco.graph_matching import (
    GMDatasetBatch,
    compute_metrics,
    get_kwargs,
    setup_data,
...
from gnnco.graph_matching._train.utils import build_visualization_batch
from gnnco.models import LaplacianEmbeddings
...
def compute_laplacian_performances(
    ...
    with mlflow.start_run(run_name=run_name) as run:
        ...
        log_visualizations(
            run, gnn_model, visualization_batch_train, "train", device
        )
        log_visualizations(
            run, gnn_model, visualization_batch_val, "val", device
        )
