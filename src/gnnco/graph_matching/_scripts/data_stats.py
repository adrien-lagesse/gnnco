...
from .. import GMDataset, GMDatasetItem
...
def gm_dataset_stats(dataset: pathlib.Path, text: bool):
        ...
        for item in data:
            ...
            graph_stats["correlation"].append(
                float(
                    ...
                    / torch.sqrt(
                        item.base_graph.adj().sum() * item.corrupted_graph.adj().sum()
                    )
                )
            )
    ...
    def print_text_stats(training: bool = True) -> None:
        ...
        print(
            f"Average base graph order: {statistics.mean(graphs_stats["base_order"])}   (stdev = {statistics.stdev(graphs_stats["base_order"])})"
        )
        print(
            f"Average corrupted graph order: {statistics.mean(graphs_stats["corrupted_order"])}   (stdev = {statistics.stdev(graphs_stats["corrupted_order"])})"
        )
        print(
            f"Average base graph density: {statistics.mean(graphs_stats["base_density"])}   (stdev = {statistics.stdev(graphs_stats["base_density"])})"
        )
        print(
            f"Average corrupted graph density: {statistics.mean(graphs_stats["corrupted_density"])}   (stdev = {statistics.stdev(graphs_stats["corrupted_density"])})"
        )
        print(
            f"Average pair correlatioin: {statistics.mean(graphs_stats["correlation"])}   (stdev = {statistics.stdev(graphs_stats["correlation"])})"
        )
